...
shoes <- read_csv(file = "_Data/sampled_shoe.csv", col_names = TRUE)
shoes
...
regress_tree <- train(time_minutes ~ . , data = trainTrans3, 
                      method = "rpart", 
                      trControl = trainControl(method = "cv", number = 10),
                      #preProcess = c("center", "scale"),
                      #tuneLength = 30)
                      tuneGrid = data.frame(cp = seq(from = 0.001, to = 0.002, by = 0.00001)))
regress_tree

pred_reg.tree.train <- predict(regress_tree, newdata = trainTrans3)
pred_reg.tree <- predict(regress_tree, newdata = testTrans3)

reg.tree.train_rmse <- postResample(pred_reg.tree.train, obs = trainTrans3$time_minutes)
reg.tree_rmse <- postResample(pred_reg.tree, obs = testTrans3$time_minutes)
reg.tree.train_rmse
reg.tree_rmse
plot(regress$finalModel, uniform = TRUE, main = "Regression Tree Fit")
text(regress$finalModel, digits = 3, all = TRUE, use.n = TRUE)
